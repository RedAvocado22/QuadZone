...
export const filterProducts = (
    ...
    if (params?.colors && params.colors.length > 0) {
        filtered = filtered.filter((p) => p.colors?.some((color) => params.colors?.includes(color)));
    }
...
export const filterOrders = (
    ...
    if (params?.search) {
        ...
        filtered = filtered.filter(
            (o) =>
                o.orderNumber.toLowerCase().includes(searchLower) || o.customerName.toLowerCase().includes(searchLower)
        );
    }
