...
export const ordersApi = {
    ...
    getMyAssignedOrders: async (
        ...
        const response = await API.get<PagedResponse<OrderResponse>>(
            `/orders/shipper/my-orders?${queryParams.toString()}`
        );
        return response.data;
    },

    updateOrderStatus: async (orderId: string | number, status: OrderResponse["status"]): Promise<OrderResponse> => {
        const response = await API.put<OrderResponse>(`/orders/shipper/${orderId}/status`, {
            orderStatus: status
        });
        return response.data;
    },
    ...
    getMyOrders: async (
        ...
        const response = await API.get<PagedResponse<OrderResponse>>(`/orders/my-orders?${queryParams.toString()}`);

        // Adapt backend PagedResponse -> frontend SimplePagedResponse
        const paged = response.data;
        return {
            data: paged.content ?? [],
            total: paged.page?.totalElements ?? 0,
            page: paged.page?.number ?? page,
            pageSize: paged.page?.size ?? pageSize
        };
    },
