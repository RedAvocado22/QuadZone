...
const ProductDetailPage = () => {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      getReviewsByProduct(Number(id), 0, 5),
    ])
      .then(([prod, revs]) => {
        setProduct(prod);
        setReviews(revs);
        ...
        <div className="tab-content p-4 border border-top-0 rounded-bottom shadow-sm">
          ...
          <div className="tab-pane fade show active" id="description" role="tabpanel">
            {product.isActive ? (
              ...
              <p>{product.description || product.modelNumber || product.brand || "No detailed description available."}</p>
            )}
          </div>
                ...
                <ul className="list-group list-group-flush">
                  {reviews.content.map((rev) => (
                    <li key={rev.id} className="list-group-item">
                      ...
                      <p className="mb-0">{rev.reviewText}</p>
                    </li>
                  ))}
                </ul>
              ) : (
