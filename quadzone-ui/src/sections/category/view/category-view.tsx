    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            if (isCategoryPage) {   
                ...
                const totalCount = res.total ?? categoryData.length ?? 0;
                setTotal(totalCount);
            } else {
            ...
            try {
                ...
                if (Array.isArray(subs)) {
                    const mappedSubs: SubcategoryRow[] = subs.map((s) => ({
                        ...
                        active: Boolean((s as any).active ?? (s as any).enabled ?? (s as any).isActive ?? true)
                    }));
                    allSub = allSub.concat(mappedSubs);
                }
            } catch (err) {
                console.error(`Error fetching subcategories for category ${cat.id}:`, err);
                // Continue with next category instead of throwing
            }
        }
