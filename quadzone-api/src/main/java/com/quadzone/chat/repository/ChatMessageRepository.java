...
public interface ChatMessageRepository extends JpaRepository<ChatMessage, Long> {
    ...
    @Query("SELECT YEAR(cm.sentAt), MONTH(cm.sentAt), COUNT(cm) FROM ChatMessage cm WHERE cm.sentAt BETWEEN :from AND :to GROUP BY YEAR(cm.sentAt), MONTH(cm.sentAt) ORDER BY YEAR(cm.sentAt), MONTH(cm.sentAt)")
    java.util.List<Object[]> aggregateMonthlyMessages(@Param("from") LocalDateTime from, @Param("to") LocalDateTime to);
}
